...
ラベルがある場合:
```bash
gh pr create --title "[タイトル]" --base main --body "[ステップ1で読んだテンプレートのセクションに内容を埋めた本文]" --assignee @me --label "label1,label2"
```

ラベルがない場合:
```bash
gh pr create --title "[タイトル]" --base main --body "[ステップ1で読んだテンプレートのセクションに内容を埋めた本文]" --assignee @me
```
