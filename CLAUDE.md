...
### ライブラリのインストール
- 新規ライブラリ追加（`npm add <pkg>` / `pnpm add <pkg>` / `composer require <pkg>` / `pip install <pkg>` 等）は必ずユーザーの承認を得る
- ロックファイルからの単純復元（`npm install` / `pnpm install` / `composer install` などパッケージ名なし）は承認不要
